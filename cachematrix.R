...
## The followings are a pair of functions will first create a cache matrix object, then calculate its inversion with cacheing ability.
...
makeCacheMatrix <- function(x = matrix()) {
  ...
  set <- function(y) { 
    x <<- y     ## Assign the new matrix value to x
    s <<- NULL  ## Re-set s value, so it can store the inverted matrix for new matrix value (y)
  }
